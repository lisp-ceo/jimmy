...
#define JIMMY_VERSION "0.0.1"
#define ABUF_INIT {NULL, 0}
#define BORDER "jimmy_edit |"
...
struct editorConfig {
  ...
  int screencols;
  int numrows;
  erow *row; // facilitated by typedef, otherwise struct erow
...
int getWindowSize(int *rows, int *cols)
{
  ...
  if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
    // Fallback if ioctl not supported
    // Sending 2 cursor command codes -> cursor forward, cursor down
    // argument is large value (999)
...
void editorRefreshScreen()
{
  ...
  abAppend(&ab, "\x1b[?25l", 6); // Hide the cursor to avoid flashing during repainting
  //  abAppend(&ab, "\x1b[2J", 4); // more optimal to clear each line as its redrawn
  abAppend(&ab, "\x1b[H", 3);
...
void editorDrawRows(struct abuf *ab)
{
  ...
  for (y = 0; y < E.screenrows; y++) { // get height of terminal?
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 9 && y == E.screenrows / 3) {
        char welcome[80];
        int welcomelen = snprintf(welcome,
                                  sizeof(welcome),
                                  "Welcome to my text editor, cunt. Version: %s", JIMMY_VERSION);
        ...
        abAppend(ab, BORDER, sizeof(BORDER) / sizeof(char));
      }
    } else {
